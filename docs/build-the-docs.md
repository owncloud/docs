...
The difference when running `antora` versus `antora-local` is, the latter command already defines a
local URL the documentation is built on. Please also see: [Overwrite the Default URL](#overwrite-the-default-url) below.
...
**NOTE:** You may get a different screen telling in the main middle frame "Page not found". In this case you have to select the manual to view. You find the selectable list on the left side. For owncloud Server, select _master_ as you usually start working there.
