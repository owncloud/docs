...
The following example shows how to create a personal mount and have it mounted automatically every time you boot your Linux computer.

. Install the `davfs2` WebDAV filesystem driver, which allows you to mount WebDAV shares just like any other remote filesystem. 
  Use this command to install it on Debian/Ubuntu:
+
----
sudo apt-get install -y davfs2
----
. Use this command to install it on _CentOS_, _Fedora_:
+
----
sudo yum install davfs2
----
+
. Use this command to install it on _openSUSE_:
----
...
----
+
NOTE: This takes effect after the next login.
+
. In your home directory, create an `owncloud` directory for the mountpoint and `.davfs2/` for your personal configuration file:
+
----
mkdir ~/owncloud
mkdir ~/.davfs2
...
----
. Make the permissions read/write for the owner only:
+
----
...
----
+ 
TIP: For more information about the file and it’s supported options, run `man 5 davfs2.conf` to access the file's man page.
...
Now, every time you boot your Linux system your ownCloud share will automatically mount, via WebDAV, in your `~/owncloud` directory. 

[IMPORTANT]
====
If the share is configured to mount automatically and (_for whatever reason_) the remote WebDAV share cannot be mounted, your server will fail to properly boot, likely leading to it entering some form of recovery mode.
When this happens, you may see boot-time errors, such as the following:

[source,console]
----
[FAILED] Failed to mount /home/[my username]/owncloud
[DEPEND] Dependency failed for Local File System
[  OK  ] Started Emergency Shell.
[  OK  ] Stopped Dispatch Password Request to Console Directory Watch
[  OK  ] Reached target Emergency Mode.
[FAILED] Failed to start Dispatch Password Request to Console Directory Watch
...
...
For that reason, it may be better to instead always set the partition to `noauto` and mount the partition toward the end of the boot process or at login time.
To find out more information about mounting partitions toward the end of the boot process in either SysVInit or systemd, please refer to the links below:

- https://fedoramagazine.org/systemd-unit-dependencies-and-order/[systemd: Unit dependencies and order]
- https://en.wikipedia.org/wiki/Init#SYSV[SysVInit]
====
