...
[IMPORTANT]
====
If the share is set to mount automatically and (_for whatever reason_) your ownCloud server fails to start, your server will fail to properly boot.
...
For that reason, it may be better to instead always set the partition to `noauto` and mount the partition toward the end of the boot process.
