...
[IMPORTANT]
====
If the share is set to mount automatically and, for whatever reason, ownCloud fails to start your server will fail to properly boot.
...
For that reason, it may be better to instead always set the partition to `noauto` and use a late in the boot process init script to mount the partition instead.
====
