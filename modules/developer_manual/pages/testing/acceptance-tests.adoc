...
The part system allows us to divide the test run without knowing how many or which suites are available. Filter tags can also be used to run targeted parts after divisions.
...
Execute the tests by setting the above environments and running the usual make command.
[source,bash]
----
...
If `run.sh` is used directly to run acceptance tests, part system can be achieved using `--part` flag. The script below divides the test suite into five parts and runs just the first one.
