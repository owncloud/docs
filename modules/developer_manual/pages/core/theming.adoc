...
For the impatient, who want to play with themes right away, the following 8 commands get a theme installed quickly. Adjust the names and paths according to your setup. For docker installations use the directory `/var/www/owncloud/custom`.
...
# 1. If it exists, go into your `apps-external` folder, else use `apps`
...
# 8. Exclude your new theme from integrity checking
...
`/owncloud`. If you’re following this guide to create or customize a
theme, make sure you change any references to match the location
of your ownCloud installation.

To save time and effort, you can use the shell script below to
...
If you are going to publish the theme as an app in the
{oc-marketplace-url}[marketplace], you need to sign it.
...
If your image size is 1920px x 1680px, you don't need the lines below the path. 
...
If you change the names, adjust `styles.css` file accordingly, as in the following example:
