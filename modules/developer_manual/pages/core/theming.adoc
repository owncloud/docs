...
[NOTE]
====
When using custom filetype icons in a custom theme, it is necessary to run `occ maintenance:mimetype:update-js` to activate them. 
For more information please refer to xref:administration_manual:configuration/mimetypes/index.adoc[mimetypes management].
====
