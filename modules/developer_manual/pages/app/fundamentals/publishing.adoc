...
image:app/app-tile-enterprise.jpg[The ownCloud 'Enterprise' Tag]
...
| license
a| At the moment following license are available:

- https://en.wikipedia.org/wiki/Open_Content_License[OCL]
- https://owncloud.com/licenses/owncloud-commercial/[ownCloud Commercial License]
+
NOTE: This is for ownCloud Enterprise Apps *only*
- https://en.wikipedia.org/wiki/Affero_General_Public_License[AGPL]
- https://en.wikipedia.org/wiki/MIT_License[MIT License].

NOTE: To overwrite a release (using the same version number) it must be in state `planned`.
Once published, you cannot replace a release.
...
| dependencies
a| Min and max version of ownCloud platform your app works with. For example:
[source,xml]
----
<dependencies>
...
NOTE: For a complete list of tags see: xref:app/fundamentals/info.adoc[].
|===
