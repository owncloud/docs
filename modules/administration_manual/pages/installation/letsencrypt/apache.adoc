...
For Apache 2.4.8 or later and OpenSSL 1.0.2 or later, you can generate and specify your {diffie-hellman-url}[Diffie-Hellman] (DH) params file. 
If not already present, add an {sslopensslconfcmd-url}[SSLOpenSSLConfCmd] directive and a new certificate with stronger keys (which improves {forward-secrecy-url}[forward secrecy]). 
...
When the certificate has been created, you can use this file in any SSL Virtual Host for which the certificate is valid, without reissuing.
...
To reduce the SSL performance penalty, we recommend you use the {sslusestapling-url}[SSLUseStapling] and {sslstaplingcache-url}[SSLStaplingCache] directives. 
...
Then adapt your Virtual Host block:
...
When successfully issuing the certificate for the first time, the certificate files exist and you can uncomment the `Include` directive to use them with SSL.
