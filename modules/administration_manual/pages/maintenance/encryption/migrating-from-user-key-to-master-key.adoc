...
NOTE: User key-based encryption blocks some additional functions such as the integration of an online editor like OnlyOffice into ownCloud. It also can cause problems when sharing files with groups. It is planned to remove this functionality from the ownCloud core in the near future. For new installations, Master key-based encryption is the recommended setup. The decryption workflow described here will only work when users have enabled password recovery and if an admin recovery password is available for the ownCloud administrator.
...
The first part of the migration process is to take is to decrypt all files and to disable encryption in ownCloud, which requires three commands to be executed.
