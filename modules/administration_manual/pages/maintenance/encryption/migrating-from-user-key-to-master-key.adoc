...
== Why should I move away from User Key Encryption

NOTE: User-key encryption is planned to be removed from Owncloud in the near future. For new installations, master key-based encryption is the recommended setup. The decryption workflow described here will only work when users have enabled password recovery and if an admin recovery password is available for the owncloud administrator.
