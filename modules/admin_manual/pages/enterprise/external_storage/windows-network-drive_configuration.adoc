...
Users have five options for login credentials:
...
==== Config-based Password

Config-based credentials provides the ability to store a password in a configuration file for accessing WND storages.
The username is the same as the user id of the ownCloud session. 
For example, the ownCloud user "Alice" with the password `mysecret` (or even without a password) uses `Alice` as their username to obtain access to the WND storage. 

The password is fetched from a key inside a `config.php.` file.
Even if the account doesn't have a password (`oAuth`, `OpenIDConnect`, or another authentication mechanism), the connection with the storage will use the password from the configuration file.
 
NOTE: The password is the same for all users.

Once this authentication mechanism is selected, a new field is available in the mount configuration, in order to set the configuration key which we want to use. 
You can create a new configuration key for this, or you can reuse any other configuration key that could be in the `config.php.*` file in the `config` directory.
For complex cases where the key is inside several nested arrays, you can use something like `<key1>#<key2>#<key3>` to get the value from that nested array. 

TIP: Numeric values as keys might be treated as strings.

[source,php]
.Example of configuring global credentials in a config.php. file.
----
'customApp.config' => [
    'server1' => [
        'host' => 'myhost',
...
Using the example above, you would use `customApp.config#server1#password` to retrieve the password for `server1` of the `customApp` to use as the password to connect to the Samba server.
