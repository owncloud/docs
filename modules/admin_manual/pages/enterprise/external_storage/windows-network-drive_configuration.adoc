...
NOTE: The listener capabilities depend on the ability of the used SMB/CIFS storage to provide notifications. Some of those, will have restrictions. Please check with your respective storage provider. Notifications for Samba only works for the target folder you're listening, but they won't go deep down. If you're listening on the "/top" folder, you won't receive notifications for "/top/middle/bottom" folder. You'd need to setup listeners for every existing folder and also for the new ones that could be created. Windows doesn't have this limitation. You'll receive notifications for every file or folder inside the folder you're listening to.
...
`wnd:listen` without any user interaction, provide the password from
a password file.
...
removed from the file by default, unless the `--pasword-trim` option is added. The password file must be readable
