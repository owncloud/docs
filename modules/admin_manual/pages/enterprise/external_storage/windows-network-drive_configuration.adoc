...
Config-based credentials provides a universal account for accessing an ownCloud instance.
...
Even if the account doesn't have a password (`oAuth`, `OpenIDConnect`, or another authentication mechanism), the connection with the storage swill use the password from the configuration file.
