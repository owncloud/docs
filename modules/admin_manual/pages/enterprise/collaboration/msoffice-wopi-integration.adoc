...
If you open a document with Microsoft Office Online in ownCloud, then it makes use of the WebDAV file locking functionality, which we have in the server. 
...
Other users can make changes, e.g., with the desktop client, but those will create conflict files for them which can be resolved afterwards.
When you close the document, Microsoft Office Office Online also has a feature to unlock the file again so it can be edited by others again as you are done with it.
