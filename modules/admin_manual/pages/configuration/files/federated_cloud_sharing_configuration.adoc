...
If the instance needs to go through a proxy, in `config/config.php`, set the `proxy` and `proxyuserpwd` configuration variables.
`proxy` sets the proxy’s hostname. 
`proxyuserpwd` sets the username and password credentials in `username:password` format, for authenticating with the proxy.
