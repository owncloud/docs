...
And then there are others that need to go through proxies which require authentication.

In both of these cases, set the `proxy` and `proxyuserpwd` configuration variables, in `config/config.php`.
