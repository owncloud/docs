...
On Ubuntu you can disable this mod by running this command:

....
a2dismod reqtimeout
....

On CentOS you have to edit the `/etc/httpd/conf/httpd.conf`. 

Find the lines starting with **LoadModule**, search for this one: 

`LoadModule reqtimeout_module modules/mod_reqtimeout.so` and comment it. Restart Apache afterwards.
