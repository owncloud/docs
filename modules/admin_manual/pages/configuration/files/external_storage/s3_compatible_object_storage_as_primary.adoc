...
NOTE: OpenStack Swift has been deprecated at all.
...
2. ownCloud, in "object store" mode as primary storage access, expects exclusive access to the object store container, because it only stores the binary data for each file. While in this mode, ownCloud stores the metadata in the local database for performance reasons.
