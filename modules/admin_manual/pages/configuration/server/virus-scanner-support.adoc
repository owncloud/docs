...
The information is then parsed, or an exit code is evaluated if no result is available to determine the response from the scan. In case of an infected upload, the approproate action is always delete. The choice "delete" or "log" for the infected condition only applies to the special case of background scans.
...
. Install Kaspersky ScanEngine based on their {kasperski-scanengine-url}[instructions] an prepare KAV for running in ICAP mode.
