...
include::{partialsdir}configuration/server/occ_command/background_jobs_selector.adoc[leveloffset=+1]
...
user (often, `www-data` or `wwwrun`) you should set up the following Cron job to call the occ
...
NOTE: The cronjob can also be placed under `/etc/cron.d`. E.g. the owncloud ansible role deploys it like this:
[source,console]
----
# cat /etc/cron.d/owncloud
USER="www-data"
#Ansible: oc cron
*/15 * * * * www-data /usr/local/bin/occ system:cron
#Ansible: occ cleanup chunks
0 2 * * * www-data /usr/local/bin/occ dav:cleanup-chunks
----
