...
Firstly, the default method is xref:ajax[AJAX], but the preferred way is to use xref:cron[Cron]. 
...
By registering your ownCloud `cron` address as an external webcron service (for example, with http://www.easycron.com/[easyCron]), you ensure that background jobs are executed regularly. 
To use this type of service, your external webcron service must be able to access your ownCloud server using the Internet. 
For example:

----
URL to call: http[s]://<domain-of-your-server>/owncloud/cron
----

=== AJAX

The AJAX scheduling method is the default option. 
However, it is also the _least_ reliable. 
...
This is done by running `php occ system:cron` multiple times. 
...
But, the most direct way to do so, is by opening three console tabs and in each one run `php occ system:cron`. 
