...
encryption
 ...
 encryption:hsmdaemon                hsmdaemon tool
...
`encryption:hsmdaemon` provides the ability to interact with an {hsm-overview-url}[hsm (Hardware Security Module)] which is integrated with ownCloud.

NOTE: This command requires ownCloud to be integrated with xref:configuration/server/security/hsmdaemon/index.adoc[hsmdaemon], which requires ownCloud Enterprise Edition >= 10.2.
For more information about hsmdaemon, please contact {owncloud-support-url}[ownCloud support].
...
|===
| `--export-masterkey` 
a| This command exports a base64-encoded representation of an ownCloud server’s master key private key file, which is used by its default encryption module.
For example, `<path/to/your/owncloud/installation>/data/files_encryption/OC_DEFAULT_MODULE/master_4a4ef7c9.privateKey`.
If a key is set, you should expect to see output similar to the following:

[source,console]
----
// Base64-encoded below output has been truncated for readability.
current masterkey (base64 encoded): 'SEJFR0lOOmNpcGflcjpBRVMtMjU2LUNUUjp'
----

If a master key is not set, then you will see the following output:

[source,console]
----
current masterkey (base64 encoded): ''
----

NOTE: This command is not required to be run.
| `--decrypt <value to decrypt>` 
...
[source,console,subs="attributes+"]
----
{occ-command-example-prefix} files_external:list \
	--mount-options \
	--output=json_pretty
[
    {
