...
NOTE: Regardless of operating system, we recommend that you enable verbose logging in both `clamd.conf` and `freshclam.conf` until you get any kinks with your ClamAV installation worked out.
...
Once ClamAV is installed, select menu:Settings[General (Admin)] and set your ownCloud logging level to "_Everything (fatal issues, errors, warnings, info, debug)_" in the "*Log*" section.

image:apps/files_antivirus/antivirus-logging.png[image]
...
The Antivirus App will show one of three warnings if it is either misconfigured or ClamAV is not available. 
...
First, set "*Mode*" to "*Daemon*".
Then, you need to set the hostname or IP address of the remote server running ClamAV and the server's port number.
...
Then, set "*Path to clamscan*" to the path to `clamscan`, which is the interactive ClamAV scanning command, on your server.
