...
NOTE: The desktop and mobile apps (clients) have a default client ID and secret hardcoded, used for ownCloud's oauth2 app. When using Kopano as IDP, it does not predefine a client ID and secret. You can use the default ones from the client to configure Kopano properly. Some IDP's like MS-Azure, these and other required parameters come from the IPD and must be coded into the client. Note that each IDP has different requirements. Get in touch with ownCloud for a branding subscription to customize the clients according your needs.
