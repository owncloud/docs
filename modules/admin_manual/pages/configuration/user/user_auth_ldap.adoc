...
It is supplied by `php7.1-ldap` on Debian/Ubuntu and `php-ldap` on CentOS/Red Hat/Fedora. Please check for the correct version, based on your installation of PHP.
...
Only those object classes::
  ownCloud will determine the object classes that are typically available for user objects in your LDAP. 
  ownCloud will automatically select the object class that returns the highest amount of users. 
...
In order to efficiently look up the groups a user who is a member of the LDAP server must support a `memberof-overlay`. 
...
You must make sure that the attribute of your choice can be fetched for both users and groups and it is unique. 
Leave it empty for default behavior. 
...
Because of this, the setting should be applied before putting ownCloud in production use and clearing the bindings (see xref:user-and-group-mapping[the User and Group Mapping` section] below).
...
Additionally the DN is cached as well to reduce LDAP interaction, but it is not used for identification. 
...
Those mappings are done in the database table `ldap_user_mapping` and `ldap_group_mapping`. The user name is also used for the user's folder (except if something else is specified in _User Home Folder Naming Rule_), which contains files and meta data.
