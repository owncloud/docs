...
TIP: We recommend xref:configuration/server/background_jobs_configuration.adoc#cron-jobs[creating a Cron job], to automate regularly syncing LDAP users with your ownCloud database. You can use the xref:developer_manual:core/apis/ocs/user-sync-api.adoc[the OCS User Sync API] to simplify the process.
