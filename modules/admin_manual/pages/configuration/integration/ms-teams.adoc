= Microsoft Teams Integration in ownCloud
:toc: right
:toclevel: 3
...
xref:configuration/user/oidc/ms-azure-setup.adoc.adoc[Example Setup Using Microsoft Azure] to configure Microsoft Azure AD and OpenID Connect.
...
. In {appsource-url}[Microsoft Teams AppSource], search for *ownCloud Generator for Admins* and open the application.

. Follow the guided instructions step by step.

. Think carefully about how to name your app. It *cannot* be changed later on. We recommend naming it *ownCloud for Teams* for easy identification.

. The ID´s to be enterd *must* be the:
.. xref:configuration/user/oidc/ms-azure-setup.adoc#client-id[CLIENT-ID] from Microsoft Azure.
.. URL how you access your owncloud instance like `+https://cloud.example.com+`.
