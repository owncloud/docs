...
* Create a directory where NGINX will save the internal images. 
  Use any path that fits to your environment. 
  Replace `/opt/cachezone` in this example with your path created:
...
if ($request_uri ~* "/apps/files_mediaviewer/")     { set $skip_cache 0; }
