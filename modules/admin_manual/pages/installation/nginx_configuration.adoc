...
* NGINX configuration blocks inherit `add_header` directives from their enclosing blocks, so you just need to
place the `add_header` directive in the top‑level server block. There’s one important exception:
...
The position of a http://nginx.org/en/docs/http/ngx_http_core_module.html#location[location]
directive connected to the result of the `map` directive is important for success.

TIP: For better readability, it is possible and advised to move _common_ `add_header`
...
NOTE: The directives `fastcgi_ignore_headers X-Accel-Buffering;` and `fastcgi_buffering off;`
...
The configuration assumes that ownCloud is installed in `/var/www/owncloud` and is accessed via
...
The configuration assumes that ownCloud is installed in `/var/www/owncloud` is accessed via
`http(s)://example.com/owncloud` and that you have `'overwriteweb root' => '/owncloud',`
...
https://central.owncloud.org/t/htaccesstest-txt-errors-in-logfiles/831[client denied by server configuration: /var/www/owncloud/data/htaccesstest.txt], or
...
When using, for example, the Gallery App, any access to a thumbnail of a picture will be logged. 
This can cause a massive log quantity making log reading challenging. 
With this approach, you can prevent access logging for those thumbnails.
...
[[configure-nginx-to-use-caching-for-owncloud-internal-images]]
=== Configure NGINX to use caching for ownCloud internal images
...
* Create a directory where NGINX will save the internal images, using any path that fits to your environment. 
  Replace `/opt/cachezone` in the following example with the path that you created.
