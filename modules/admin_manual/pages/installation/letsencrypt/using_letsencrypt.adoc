...
supported_, and this page is _community-maintained_.
...
to your servers IP address. In case your server is behind a firewall, take
the necessary measures to ensure that your server is accessible, worldwide,
...
NOTE: This method is preferred when you’re using a version of Ubuntu prior to 18.04.
...
[NOTE]
====
Depending on how you installed Let’s Encrypt, certbot may also be named `letsencrypt` or
`certbot-auto`. However, this guide will refer to it as `certbot`. Please bear that in
mind, and update the examples and scripts used in this guide to reflect your Certbot
installation.
====
...
Now that Certbot is installed, register your email address for urgent renewal
and security notifications. This command also prepares Certbot’s environment
if it’s not already installed. To do this, run the following command:
...
installation. +
You can find it by running `which certbot`.
...
This file defines some default settings used by Certbot. Use the email address
...
* In case you have enabled the post hook for your webserver in `cli.ini`, it will reload
the web server configuration automatically if a certificate has been renewed.
...
[NOTE]
====
You can create different certificates for different sub-domains, such as
`example.com`, `www.example.com`, and `subdomain.example.com`, by creating different scripts.
====
...
If you want to list (view) existing SSL certificates, use `list.sh`,
which can be run as follows:

[source,console]
----
sudo sh /etc/letsencrypt/list.sh
----
...
you renew them before that time. +
...
[source,console]
----
sudo sh /etc/letsencrypt/renew.sh
----
...
NOTE: If you want to use own values, you can check them eg. at
https://crontab.guru[crontab.guru] or modify the script for other options.

== Add Extra Domains to the Certificate

If you want to add an extra domain, like `subdomain.example.com`, to your
...
NOTE: This also implies that you need to comment the `include` directive
(please refer to the relevant xref:web-server-setup[web server setup])
and follow the steps afterwards.
...
[source,console]
----
sudo sh /etc/letsencrypt/delete.sh
----
