...
We recommend creating a separate file for storing the `SSL` directives for each site. If these directives already exist in this Virtual Host for the site, delete them and include the file instead. This is because when the certificate has been created, you can use this file in any SSL-enabled VirtualHost configuration for which the certificate is valid without reissuing the SSL certificate. It also eases the management for the web site certificate files, as you can easily in- or exclude the file in the virtual config with a single remark and it keeps the files contents compact. 
