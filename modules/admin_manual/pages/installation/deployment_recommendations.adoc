...
https://mariadb.com/kb/en/setting-up-replication/[master-slave replication].
InnoDB storage engine, MyISAM is not supported, see:
xref:configuration/database/linux_database_configuration.adoc#mysql-mariadb-storage-engine[MySQL / MariaDB storage engine].
For mariadb consider: https://mariadb.com/kb/en/mariadb-maxscale-22-automatic-failover-with-mariadb-monitor[MariaDB Monitor] to configure your setup for a failover Scenario.
