...
You can install any purchased or free certificate like one via Let's Encrypt.

Read the
xref:installation/letsencrypt/index.adoc[Using Let's Encrypt SSL Certificates]
if you want to use a free of charge certificates for Apache.
...
NOTE: Self-signed certificates have their drawbacks - especially when you plan to make your ownCloud server publicly accessible. You might want to consider getting a certificate signed by a commercial signing authority or a free certificate like one via Let's Encrypt.
