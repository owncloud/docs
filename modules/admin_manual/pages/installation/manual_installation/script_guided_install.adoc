...
It is mandatory to run this script with root previleges, because files and directories will be created and ownwership and permissions will be set. Call the instance script like:
...
the old instance path is backed up by renaming and adding a time stamp. A new target folder with the old name is created. This ensures in case of extracting and preperation issues, you can easily go back to the previous version. After a successful upgrade, you must manually remove the backup folder.
