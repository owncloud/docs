...
ownCloud can be installed using Docker, using the {dockerhub-url}[official ownCloud Docker image].
...
owncloud_mariadb   docker-entrypoint.sh --max ...   Up (healthy)   {std-port-mysql}/tcp
owncloud_redis     docker-entrypoint.sh --dat ...   Up (healthy)   {std-port-redis}/tcp
owncloud_server    /usr/bin/entrypoint /usr/b ...   Up (healthy)   0.0.0.0:std-port-http}->std-port-http}/tcp
----
...
[source,console]
----
sudo docker-compose exec mariadb /usr/bin/mysqldump -u root --password=owncloud owncloud > owncloud_$(date +%Y%m%d).sql
----
+
