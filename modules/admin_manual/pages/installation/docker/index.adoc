...
NOTE: With 10.5 the `image: registry.owncloud.com/owncloud/enterprise` is deprecated. All supported enterprise features and apps are now included in the public `image: owncloud/server`. A login to our registry `docker login registry.owncloud.com` is no longer needed. 
