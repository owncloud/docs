...
:toc: right
:docker-compose-file-url: https://raw.githubusercontent.com/owncloud/docs/master/modules/admin_manual/examples/installation/docker/docker-compose.yml
...
To use it, first create a new project directory and download {docker-compose-file-url}[ownCloud's docker-compose.yml file] into that new directory. 
...
`docker-compose down --rmi all --volumes` to stop and remove containers, along with the
