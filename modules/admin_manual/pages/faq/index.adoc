...
There are known file names that can not be synched with ownCloud

- Folder or file with a space at the end  (trailing spaces)
- .htaccess
- `*.part` files
- too long filenames (more than 253 characters)
- https://github.com/owncloud/client/blob/master/sync-exclude.lst[client/sync-exclude list]
- Desktop.ini in root
- Files with leading dot (example `.12345.pdf`) are handled as hidden files on all plattforms. (user need to activate "sync hidden files" to sync them)
