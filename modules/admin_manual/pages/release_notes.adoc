...
ownCloud Server 10.1.1 is a hotfix follow-up release that takes care of an issue with when loading updated apps. Instead of updating the app versions to their new values in the database, the old version value is written causing the process to repeat with every request. (https://github.com/owncloud/core/issues/34851)

This issue can cause high load on the database, especially in large installations. If you have already upgraded to 10.1.0, please consider upgrading to 10.1.1. You can expect minimal downtime as it's not necessary to run `occ upgrade` after updating the code to the new version.

Apart from this fix, please consider the ownCloud Server 10.1.0 release notes.
