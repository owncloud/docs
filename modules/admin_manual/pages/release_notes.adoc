...
These are designed to cope with issues encountered around duplicate session tokens, which make the ownCloud Clients lose their OAuth2 authorization from time to time, and forcing users to reauthenticate. 
The session handling stability has been generally improved, and support for Redis Session Locking has been introduced. 
If Redis is used, it is necessary to enable Session Locking to ensure that the mentioned issues no longer occur. 
...
* After enabling `files_external_s3` (`occ app:enable files_external_s3`) it is necessary to run `occ upgrade` again to conduct config migrations. 
  This migration step should run quickly.
* If users were allowed to configure personal mount points before the upgrade, switch from maintenance mode into single user mode (`occ maintenance:singleuser --on`) and enable the option again by ticking the respective checkbox below "_Allow users to mount external storage_" (in Admin settings => Storage).
...
* When loading groups, the performance of users has been improved https://github.com/owncloud/core/pull/35822[#35822]
* Improved memory handling for the trashbin expiry background job https://github.com/owncloud/core/pull/35708[#35708]
...
* *User/group sharing permission handling:* When a share recipient shared a resource with a group the resource owner was a member of (reshare), the resource owner was unable to increase the permissions of the initial share. 
   This has now been fixed. 
   https://github.com/owncloud/core/pull/35884[#35884]
* *User/group sharing permission handling:* When a user shared a resource with a group, share recipients (members of the group) were able to remove the share altogether (instead of just unsharing from themselves). 
   This has been fixed. 
   https://github.com/owncloud/core/pull/36120[#36120]
...
* A new Roles API has been added to allow clients to query the server for available permissions/roles for user/group sharing and public links. 
  In future client releases, this endpoint will be used to dynamically display roles/permissions depending on the server's capabilities. https://github.com/owncloud/core/pull/36024[#36024]
Docs: https://github.com/owncloud/docs/issues/1857
* A new, improved version of the "Advanced Sharing Permissions" API (v2) has been added to allow ownCloud apps to register additional permissions/restrictions in user/group sharing. 
