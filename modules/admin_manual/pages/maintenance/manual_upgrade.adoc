...
First, backup ownCloud and the server database as described in section xref:maintenance/backup.adoc[Backing up ownCloud].
...
Download the latest https://owncloud.org/download/#owncloud-server-tar-ball[ownCloud server release] to where your previous installation was, in this example the default default directory `/var/www/`.
[source,console,subs="attributes+"]
----
cd /var/www/
...
NOTE: In this description we assume that your ownCloud installation was located in the default directory: `/var/www/owncloud` and the new release will do so as well. 
...
[source,console,subs="attributes+"]
----
tar -xf owncloud-{latest-download-version}.tar.bz2
----
