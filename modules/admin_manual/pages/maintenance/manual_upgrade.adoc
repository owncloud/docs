...
NOTE: How to get the correct link to the tarball

. Login to https://customer.owncloud.com with your enterprise credentials 
. Navigate to the tarball you want to download 
. Right click on the name of the file and click:
** menu:Copy Link Address[] in Google Chrome
** menu:Copy Link Location[] in Firefox
** menu:Copy Link[] in Safari

This, together with your credentials, is the link you need for the `wget` command.
