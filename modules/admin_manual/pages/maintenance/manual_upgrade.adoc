...
[TIP]
====
The code snippet below retrieves the list of core apps.

[source,console]
----
domain="https://marketplace.owncloud.com/ajax/publishers/owncloud/products"
curl "${domain}" --silent  --compressed \
  | jq '.[].title' \
  | sed 's/"//g' \
  | sort
----
