...
NOTE: The script is most convenient if you have your `apps`, `apps-external` and your `data` directory linked, as it takes care of re-linking - you will get asked about when using. When having fixed directories, these are created, but content must be moved/copied manually before finalizing the upgrade. When not using the `apps-external` directory, you must manually take care copying only those apps which are not part of the new source.
...
.Set strong permissions for the .haccess files
