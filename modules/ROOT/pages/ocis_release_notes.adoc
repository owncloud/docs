...
This list view provides additional information as name of the
...
+
These colors ensure, that the primary and outlined buttons (eg. "Upload" and "New") have a proper hover state.
- If you used `yes` or `no` in your YAML files, please change these values to `true` or `false`. Background: The YAML parser we use does not suport `yes` or `no`` any more.

=== Breaking Changes

- Usage of Web UI and ownCloud 10 as backend is not recommended starting with version 7.1.0. of the Web UI. If you want to use the Web UI with ownCloud 10, please use App version 7.0.2 . Critical bugs can be fixed upon request. 
