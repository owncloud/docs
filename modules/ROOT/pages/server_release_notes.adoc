...
When upgrading from ownCloud Server 10.10, there will be no migrations steps impacting the upgrade duration. Therefore the upgrade is expected to finish quickly.
...
* The default app on login can now be set per individual user. ownCloud Web will use this to remember which UI the user has used during their last session. This way, users are always redirected to their preferred UI after logging in. https://github.com/owncloud/core/pull/39600[#39600]
