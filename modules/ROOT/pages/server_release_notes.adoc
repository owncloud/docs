...
User-key encryption has been already deprecated in core versions later than 10.7. For this reason, the command line interface and web UI to enable user-key encryption is no longer available in ownCloud core 10.12.1 + encryption app 1.6.0 or later. If you are operating an ownCloud installation with user-key encryption enabled, please get in contact with support@owncloud.com to plan a migration to master-key encryption. https://github.com/owncloud/encryption/pull/389[encryption#389] 
