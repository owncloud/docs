...
=== Fix .htaccess with index.php-less servers

With 10.12.0 we introduced a regression in our .htaccess settings. In the case of index-php-less setup combined with a subdirectory, uploads via webdav would fail with "405 Method Not Allowed". https://github.com/owncloud/core/issues/40696


=== Fix location of Uploads folder

With 10.12.0 we introduced a regression regarding the temporary uploads folder. When using the home_folder_naming_rule attribute for defining the home directories for LDAP users (configurable over the LDAP wizard) chunks of users' uploads are wrongly created under the default data directory rather than inside the configured home directory. https://github.com/owncloud/core/pull/40693
...
Quota calculation on 32 bit systems sufferend from an inconsistent use of float and integer data types. The fix now correctly supports that the size could be either an integer or a float. https://github.com/owncloud/core/pull/40729
