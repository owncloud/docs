...
* xref:changes-in-10-13-3[Changes in 10.13.4]
...
We now display IoC scanner instructions to all Customers (a valid license key needs to be present) during upgrade (console as well as Web Updater) and in the admin settings. Additionally, a new config.php option `indicators-of-compromise-scanner.confirmation` has been introduced which gives admins the possibility to suppress the instrcutions once the script has been correctly run on ALL ownCloud servers. https://github.com/owncloud/core/pull/41137[#41137]

Background: the Indicators of Compromise (IoC) tool plays a vital role in identifying potential security threats or breaches. The tool analyzes your ownCloud 10 deployments and determines whether they have been compromised via the known vulnerabilities. It collects information from the Apache logs and identifies the signatures of potential exploits. As mentioned above, the tool has to be run ALL ownCloud servers in case of a clustered setup.

=== 2FA check on controllers which are annotated as @PublicPage and also used authenticated

We now check 2FA on controllers which are publicly accessible and authenticated. https://github.com/owncloud/core/pull/41123[#41123]
