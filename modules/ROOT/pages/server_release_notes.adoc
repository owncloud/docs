...
* xref:changes-in-10-13-0[Changes in 10.13.1]
...
The "Open in Web" feature for direct editing of office documents now properly works on iOS and Desktop clients with Collabora. 

=== Avoid Loading 3d-party Resources

The "Open in Web" configuration offers icons to be shown in the clients. This is e.g. used by the iOS 12.0.3 client. We now ship the icons embedded in core.

=== Notable Changes

* Fix: disallow pre-signed url access if the signing key is not initialized.
* New code was added to dismiss invalid settings of the redirection endpoint URI as seen in the OAuth2 protocol, according to RFC#7636.
...
* Two cron jobs related to cleaning up left over thumbnails were not properly enabled in the past (`PreviewCleanupJob`). In 10.13.0 the code was fixed, and the jobs startd running. However, in certain cases with an exceptional high number of potential cleanups, these jobs can now take up a substantial amount of database resources. We propose to temporarily disable the offending job until we have a fix.
