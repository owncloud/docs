...
2-factor authentication can be enforced now. The feature requires at least one app implementing the 2-factor authentication, otherwise no enforcement will be done. If the 2-factor authentication is enforced, all users will be required to use a 2-factor authentication app. Some specific groups selected by the admin can be excluded to let those users bypass the 2-factor authentication. https://github.com/owncloud/core/pull/40830[#40830]

=== Setting up federation sharing over the WebUI is again possible

After an update of the guzzle library in previous core versions, it was no longer possible to set up federation sharing over the WebUI because of the wrong format used during the token exchange phase. We now explicitly set the format to be json so that the exchange is correctly done. https://github.com/owncloud/core/pull/40815[#40815]
...
When the "share_folder" config.php option was set, guest users could not see anymore their received shares. We now skip the "share_folder" config.php option for guest users and default to root. https://github.com/owncloud/core/pull/40864[#40864]
