...
* Requiring a downtime when used on all affected sorages at once.
...
TIP: If many users are affected, it could be conveniant to create a shell script, which iterates over a list of User ID's.
...
| `--output` | The output format to use (`plain`, `json` or `json`pretty`. The default is `plain`).
|===
