...
connected to a backend any longer. Options are `disable` and `remove`.
...
[width="100%",cols="20%,70%",]
|===
|`-u, --user=[USER]`   | User id to whom the notification shall be sent.
|`-g, --group=[GROUP]` | Group id to whom the notification shall be sent.
|`-l, --link`=[LINK]   | A link associated with the notification.
|===
